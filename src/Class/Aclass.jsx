...
import { carDetail1 } from '../data'

const Aclass = () => {
  return (
    <div className='grid sm:grid-cols-1 sm:flex-wrap md:grid-cols-4 md:grid-flow-row lg:grid-cols-3 xl:grid-cols-4 '>
      {carDetail1.map((car, index) => (
        <div  key={index} className='h-full   mx-3 mt-16  pt-6self-start rounded-lg block bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]'>
          <a href="#!">
              <img
                class="rounded-lg md:h-[240px]  bg-cover transition duration-300 ease-in-out hover:scale-95"
                src={car.img}
                alt={car.Tit} />
          </a>
          <div class="p-6">
            ...
            <button className='ml-[30px]   font-bold  w-[150px] rounded bg-blue-500 px-4 py-3 text-white hover:bg-blue-700 flex float-left '>Learn More
